...
static int pathparser_path_valid_format(const char* filename) {
    int len = strnlen(filename, LILOS_MAX_PATH);
    // first byte must be digit because we ignore all drives except 0 drive
...
static int pathparser_get_drive_by_path(const char** path) {
    ...
    *path += 3; // Adjusts the pointer it was passed to skip drive number
    return drive_no;
}
...
static const char* pathparser_get_path_part(const char** path) {
    char* result_path_part = kzalloc(LILOS_MAX_PATH);
    int i = 0;
    ...
    if (i == 0) {
        // Then we didn't actually parse anything
