...
#define LILOS_FAT16_FAT_ENTRY_SIZE 0x02
#define LILOS_FAT16_BAD_SECTOR 0xFF7
#define LILOS_FAT16_UNUSED 0x00

// following 3 lines do not represent anything on the disk
...
struct fat_directory_item {
    uint8_t filename[8];
    uint8_t ext[3];
    uint8_t attribute; // all those flags defined above
    uint8_t reserved;
...
// Basically represents an open file
...
int fat16_sector_to_absolute(struct disk* disk, int sector) {
    // Will return an absolute position for sector
    // e.g. 1st sector 512, 2nd sector 1024...
    return sector * disk->sector_size;
}

int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector) {
    struct fat_directory_item item;
    struct fat_directory_item empty_item; // without memset, this is uninitialized on the stack
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != LILOS_ALL_OK) {
        res = -EIO;
        goto out;
    }

    while(1) {
        if (diskstreamer_read(stream, &item, sizeof(item)) != LILOS_ALL_OK) {
            res = -EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory) {
    ...
    // Make sure we read all of it if it doesn't perfectly divide into sector size
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != LILOS_ALL_OK) {
        res = -EIO;
        goto err_out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != LILOS_ALL_OK) {
        res = -EIO;
        goto err_out;
    }
...
int fat16_resolve(struct disk* disk) {
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != LILOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
    ...
    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != LILOS_ALL_OK) {
        res = -EIO;
        goto out;
    }
...
static uint32_t fat16_get_first_cluster(struct fat_directory_item* item) {
    // Join low 16 bits with out high 16 bits
...
static uint32_t fat16_get_first_fat_sector(struct fat_private* private) {
    // Since the very first File Allocation Table comes right after the reserved sectors
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * LILOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
        goto out;
...
/**
 * Get the corrent cluster to use based on the starting cluster and the offset
 */
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset) {
    ...
    for (int i = 0; i < clusters_ahead; i++) {
        ...
        if (entry == LILOS_FAT16_BAD_SECTOR) {
            res = -EIO;
            goto out;
        }
        ...
        if (entry == 0x00) {
            // found no cluster... assume file allocation table is corrupted
            res = -EIO;
            goto out;
        }
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item) {
    ...
    if (!(item->attribute & FAT_FILE_SUBDIRECTORY)) {
        // We want to be passed directories, not files
        res = -EINVARG;
        goto out;
    }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name) {
    ...
    for (int i = 0; i < directory->total; i++) {
        ...
        if (istrncmp(tmp_filename, name, sizeof(tmp_filename)) == 0) {
            // Found it let's create a new item
...
int fat16_read(struct disk* disk, void* descriptor, uint32_t size, uint32_t nmemb, char* out_ptr) {
    // must return the amount of nmembs that were successfully read
    int res = 0;
    // cast generic void* type passed from VFS (which it got from fat16_open) back to its original type fat_file_descriptor
