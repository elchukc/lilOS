...
static bool elf_is_execitable(struct elf_header* header) {
    // We don't let ppl load their executables anywhere in memory they like
    return header->e_type == ET_EXEC && header->e_entry >= LILOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
struct elf_header* elf_header(struct elf_file* file) {
    // Elf memory is elf header because the very start of elf file is the header
    return file->elf_memory;
}
...
int elf_validate_loaded(struct elf_header* header) {
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? LILOS_ALL_OK : -EINFORMAT;
}
