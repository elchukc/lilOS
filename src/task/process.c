...
static struct process* processes[LILOS_MAX_PROCESSES] = {};
...
struct process* process_get(int process_id) {
    if (process_id < 0 || process_id >= LILOS_MAX_PROCESSES)
        return NULL;
...
int process_switch(struct process* process) {
    // no need to save anything before switching because we assume all
    //   processes are using the same video memory
...
static int process_terminate_allocations(struct process* process) {
    for (int i = 0; i < LILOS_MAX_PROGRAM_ALLOCATIONS; i++) {
        // Uncomment if we wanna do it the easy, slow way
        // process_free(process, process->allocations[i].ptr);
        // Alternately, this is the faster way
...
void process_switch_to_any() {
    for (int i = 0; i < LILOS_MAX_PROCESSES; i++) {
        if (processes[i]) {
            process_switch(processes[i]);
            return;
        }
    }
    // TODO respawn a shell. process_load_shell or something.
    panic("No processes to switch to.\n");
}

static void process_unlink(struct process* process) {
    // NOTE Whats the diff btwn this and calling process_allocation_unjoin
...
int process_inject_arguments(struct process* process, struct command_argument* root_argument) {
    ...
    while (current) {
        // We are currently allocating more memory than we need
...
int process_map_binary(struct process* process) {
    int res = 0;
    paging_map_to(process->task->page_directory, (void*) LILOS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
/** Note process MUST be properly intitialized with memory properly loaded in */
int process_map_memory(struct process* process) {
    ...
    // use the stack end because the stack goes downwards in x86 systems
    //      e.g. end of stack is 0x1000, start of stack is 0x5000
    paging_map_to(process->task->page_directory, (void*)LILOS_PROGRAM_VIRTUAL_STACK_ADDRESS_END, process->stack, paging_align_address(process->stack + LILOS_USER_PROGRAM_STACK_SIZE), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
out:
    return res;
}
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot) {
    ...
    program_stack_ptr = kzalloc(LILOS_USER_PROGRAM_STACK_SIZE);
    ...
    _process->task = task;
    // because we use the paging model, we must map memory
