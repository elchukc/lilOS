...
/** Must be in kernel page before entering this function */
...
void task_current_save_state(struct interrupt_frame* frame) {
    // paging must be set to kernel page before calling this
    if (!task_current()){
...
int task_init(struct task* task, struct process* process) {
    ...
    task->registers.ip = LILOS_PROGRAM_VIRTUAL_ADDRESS;
    task->registers.ss = USER_DATA_SEGMENT;
    task->registers.cs = USER_CODE_SEGMENT;
    task->registers.esp = LILOS_PROGRAM_VIRTUAL_STACK_ADDRESS_START;
