...
int paging_map_to(struct paging_4gb_chunk* directory, void* virt, void* phys, void* phys_end, int flags) {
    ...
    int total_pages = total_bytes / PAGING_PAGE_SIZE; // isn't this +1 bc its floor division?
...
int paging_set(uint32_t* directory, void* virt, uint32_t val) {
    ...
    // extract only the address from the page directory entry. Exclude flags.
    // since the lower bits aren't used when we're aligned to 4096
    //      and every page table entry is aligned to 4096
    uint32_t* table = (uint32_t*)(entry & 0xfffff000);
...
/** Takes the virtual address and returns the entry in the page table
 *    i.e. the physical address with the flags
 */
